...
class Item:
    ...
    Args:
      dic: a dictionary containing the location (x and y coordinates), color, size, shape of the item and the box to which it belongs to
    """
    ...
    @property
    def right(self):
        """Return the right coordinate of the item.
        """
        return self.__x_loc + self.size.value

    @property
    def top(self):
        """Return the top coordinate of the item.
        """
        return 100 - self.__y_loc

    @property
    def bottom(self):
        """Return the bottom coordinate of the item.
        """
    ...
    @property
    def left(self):
        """Return the left coordinate of the item.
        """
        return self.__x_loc
    ...
    def touching_right(self, use_margin = False):
        """Check if the item is touching the right.

        Args:
          use_margin (bool): allows a certain margin (consider "almost touching" as "touching")
        """
        margin = ALMOST_TOUCHING_MARGIN if use_margin else 0
        return self.right >= 100 - margin

    def touching_left(self, use_margin = False):
        """Check if the item is touching the left.
    ...
    def touching_bottom(self, use_margin = False):
        """Check if the item is touching the bottom.
    ...
    def touching_top(self, use_margin = False):
        """Check if the item is touching the top.
...
class Box:
    ...
    def __init__(self, items_as_dicts : typing.List[dict]):
        """Represents the 'box' in the image.

        Args:
          items_as_dicts: a dictionary containing the items in the box.
        """
    ...
    def is_tower(self):
        """Check whether this is an image of towers of blocks (not a regular image with objects of various shapes).
        """
...
class Image:
    """Represents the structured representation of an image.
    """
    ...
    def is_tower(self):
        """Check whether this is an image of towers of blocks or a regular image with objects of various shapes.
